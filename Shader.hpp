...
#pragma warning(pop)
    ...
    if constexpr (T == SupportedShaderType::kPixel) {
      if (HRESULT const operation_status = GetDevice(gfx).CreatePixelShader(
      ...
      if (HRESULT const operation_status = GetDevice(gfx).CreateVertexShader(
              shader_blob_->GetBufferPointer(), shader_blob_->GetBufferSize(),
              kNoClassLinkage, &shader_);
          FAILED(operation_status)) {
        throw GetDebugger(gfx).CreateDirectXError(
...
#pragma warning(pop)
    if constexpr (T == SupportedShaderType::kPixel) {
      GetDeviceContext(gfx).PSSetShader(shader_.Get(), kNoInterfaces, 0u);
    } else if constexpr (T == SupportedShaderType::kVertex) {
      GetDeviceContext(gfx).VSSetShader(shader_.Get(), kNoInterfaces, 0u);
    } else {
      std::unreachable();
    }
  }
